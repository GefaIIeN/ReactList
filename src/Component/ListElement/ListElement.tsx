...
export const ListElement = ({
    ...
    const [changedFirstNameValue, setFirstNameValue] = useState<string>(element.firstname);
    const [changedSecondNameValue, setSecondNameValue] = useState<string>(element.secondname);
    const [changedBirthdayValue, setBirthdayValue] = useState<string>(element.birthday.toLocaleDateString("ru-RU"));
    ...
    const getEditElement = () => {
        return (
            <div className="editElements">
                <input 
                    value={changedFirstNameValue} 
                    onChange={(event) => { handleChange('firstName', event.target.value) }} 
                />
                <input 
                    value={changedSecondNameValue} 
                    onChange={(event) => { handleChange('secondName', event.target.value) }}
                />
                <input 
                    value={changedBirthdayValue} 
        ...
        if (isEdit) {
            return (
                <button 
                    type="button" 
                    onClick={() => {
                        const value = createField(changedFirstNameValue, changedSecondNameValue, new Date(changedBirthdayValue), element.id);
                        onExitEdit(value);
                    }}
                >
